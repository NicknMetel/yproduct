sap.ui.define([
    "yproduct/controller/BaseController",
    ...
    function (Controller, MessagePopover, MessageItem, MessageToast, Element) {
        ...
        return Controller.extend("yproduct.controller.Create", {
            onInit: function () {
				...
				function (RTE, library) {
					...
					that.getView().byId("idEditorContainer").addItem(that.oRichTextEditor);
				});
            },
            ...
            _onObjectMatched: function(oEvent){
                ...
                oModel.metadataLoaded().then(function(){
                    var oContext = oModel.createEntry("ProductSet", null);
            ...
            handleSave: function(oEvent){
                ...
                oModel.submitChanges({
                    success: function(oData, oResp) {
                        //this._sProdid = oEntity.Prodid;
                         var oResponseData = JSON.parse(oData.__batchResponses[0].__changeResponses[0].body);
                        this._sProdid = oResponseData.d.Prodid; 

                        var oUploadSet = oView.byId("idUploadSet");
                        oUploadSet.upload();
                        MessageToast.show(oResourceBundle.getText("msgSaveSuccessfully"));
            ...
            // handleBeforeUploadStarts: function(oEvent) {
            //     var sToken = this._getFileToken(this.getView().getModel().sServiceUrl);
            //     var oUploadSet = oEvent.getSource();
            //     var oItem = oEvent.getParameter("item");

            //     oUploadSet.destroyHeaderFields(); // 필요 시 유지

            //     oUploadSet.insertHeaderField(new sap.m.UploadCollectionParameter({
            //         name: "x-csrf-token",
            //         value: sToken
            //     }));
            ...
            //     oUploadSet.insertHeaderField(new sap.m.UploadCollectionParameter({
            //         name: "slug",
            //         value: encodeURIComponent(oItem.getFileName())
            //     }));

            //     oUploadSet.insertHeaderField(new sap.m.UploadCollectionParameter({
            //         name: "prodid",
            //         value: this._sProdid // 실제 키 값
            //     }));
            // },

            // _getFileToken: function(sServiceUrl) {
            //     var sToken = "";
            //     jQuery.ajax({
            //         url: sServiceUrl,
            //         type: "GET",
            ...
            //         }
            //     });
            //     this._sUploadToken = sToken;
            //     return this._sUploadToken;
            // }
            ...
            _getFileToken: function(sServiceUrl) {
                return new Promise(function(resolve, reject) {
                    var oModel = new sap.ui.model.odata.v2.ODataModel(sServiceUrl, { useBatch: false });
                    oModel.refreshSecurityToken(function() {
                        resolve(oModel.getSecurityToken());
                    }, function() {
                        reject("CSRF Token fetch failed");
                    });
                });
            }
            ...
            //         },
            //         error: function() {
            //             console.log("_getFileToken error----------------");
            //         }
            //     });
            //     this._sUploadToken = sToken;
            //     return this._sUploadToken;
